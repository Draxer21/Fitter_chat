...
def create_preference():
    ...
    try:
        ...
        if not payer_info.get('name'):
            payer_info['name'] = current_user.full_name.split()[0] if current_user.full_name else 'Cliente'
        if not payer_info.get('surname'):
            name_parts = current_user.full_name.split() if current_user.full_name else []
            payer_info['surname'] = ' '.join(name_parts[1:]) if len(name_parts) > 1 else 'Fitter'
        ...
        current_app.logger.exception("Error al crear preferencia de pago")
...
def webhook():
    ...
    try:
        ...
        current_app.logger.exception("Error procesando webhook")
...
def payment_status(order_id):
    """
    Obtener el estado del pago de una orden
    """
    try:
        ...
        current_app.logger.exception("Error al obtener estado del pago")
        return jsonify({'error': 'Error al procesar la solicitud'}), 500
...
def order_payment(order_id):
    """
    Obtener información completa de orden y pago
    """
    current_user, error = _require_auth()
    if error:
        return error
    ...
    try:
        ...
        current_app.logger.exception("Error al obtener orden con pago")
        return jsonify({'error': 'Error al procesar la solicitud'}), 500
...
def verify_payment(payment_id):
    """
    Verificar el estado de un pago consultando a MercadoPago
    y actualizar la base de datos
    """
    try:
        mp_service = MercadoPagoService()
        result = mp_service.verify_and_update_payment(payment_id)
        ...
        current_app.logger.exception("Error al verificar pago")
