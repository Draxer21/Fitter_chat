...
class MercadoPagoService:
    ...
    def verify_and_update_payment(self, mp_payment_id):
        """
        ...
        Returns:
            dict: Información del pago actualizada
        """
        try:
            ...
            # Actualizar información del pago
            payment.payment_id = str(mp_payment_id)
            payment.status = payment_info.get('status')
            payment.payment_method_id = payment_info.get('payment_method_id')
            payment.payment_type_id = payment_info.get('payment_type_id')
            payment.merchant_order_id = str(payment_info.get('order', {}).get('id', ''))
            
            # Si está aprobado, actualizar la orden
            if payment.status == 'approved':
                from datetime import datetime
                payment.approved_at = datetime.utcnow()
            ...
            return {
                ...
                'order_status': payment_info.get('status')
            }
            
        except Exception as e:
            current_app.logger.error(f"Error verificando pago: {str(e)}")
            db.session.rollback()
            return None
