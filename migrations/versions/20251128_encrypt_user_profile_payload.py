...
def upgrade() -> None:
    ...
    existing_columns = {col["name"] for col in inspector.get_columns("user_profile")}

    added_columns = False

    if "encrypted_payload" not in existing_columns:
        op.add_column("user_profile", sa.Column("encrypted_payload", sa.LargeBinary(), nullable=True))
        added_columns = True
    if "payload_checksum" not in existing_columns:
        op.add_column("user_profile", sa.Column("payload_checksum", sa.String(length=64), nullable=True))
        added_columns = True

    if not added_columns:
        # Las columnas ya existen; asumimos que la migración de datos se ejecutó previamente.
        return
...
def downgrade() -> None:
    ...
    if "payload_checksum" in existing_columns:
        op.drop_column("user_profile", "payload_checksum")
    if "encrypted_payload" in existing_columns:
